...
def __reorder_columns(prefix: str, df_normalized: DataFrame) -> DataFrame:
    ...
    if prefix == "product":
        ...
        final_columns_order = (
            result_columns
            + grouped_columns[ColumnGroup.STATUS_TYPE_SUMMARY]
            + grouped_columns[ColumnGroup.PROPERTIES]
        )
    elif prefix == "step":
        final_columns_order = (
            step_columns
            + grouped_columns[ColumnGroup.INPUTS]
            + grouped_columns[ColumnGroup.OUTPUTS]
            + grouped_columns[ColumnGroup.DATA]
            + grouped_columns[ColumnGroup.PROPERTIES]
        )
...
def normalize_dataframe(df: List[Dict[str, Any]], prefix: str) -> pd.DataFrame:
    ...
    if prefix == "step":
        ...
        steps_merged_df = pd.merge(
            steps_df,
            steps_parameters_df,
            left_on=["result_id", "step_id"],
            right_on=["result_id", "step_id"],
            how="left",
        )
...
def merge_dataframes(
    results_df: DataFrame, steps_df: DataFrame, products_df: DataFrame
) -> DataFrame:
    ...
    """
    merged_product_result_df = pd.merge(
        products_df,
        results_df,
        left_on="product_part_number",
        right_on="result_part_number",
        how="inner",
    )
    merged_product_result_step_df = pd.merge(
        merged_product_result_df,
        steps_df,
        left_on="result_id",
        right_on="step_result_id",
        how="inner",
    )
