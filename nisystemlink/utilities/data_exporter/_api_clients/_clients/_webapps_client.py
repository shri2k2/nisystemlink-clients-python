...
from nisystemlink.utilities.data_exporter._api_clients._models._webapp_content import (
    WebappContent,
)
from nisystemlink.utilities.data_exporter._api_clients._models._webapp_meta_data import (
    WebappMetaData,
)
...
class WebAppsClient:
    ...
    def __init__(self, api_key: str | None, systemlink_uri: str | None) -> None:
        ...
        self.__query_webapps_url = (
            f"{systemlink_uri}{BaseHttpRoutes.WEBAPPS_BASE_ROUTE}/query"
        )
        self.__get_webapp_content_url = (
            f"{systemlink_uri}{BaseHttpRoutes.WEBAPPS_BASE_ROUTE}/{{webapp_id}}/content"
        )

    async def get_webapp_metadata(
        self, dataspace_name: str, workspace_id: str
    ) -> WebappMetaData:
        ...
        """
        body = {
            "filter": f'name == "{dataspace_name}" && type == "DataSpace" && workspace == "{workspace_id}"'
        }
        if self.__api_key:
            headers = {"x-ni-api-key": self.__api_key}
        webapps = await post_request(
            self.__query_webapps_url, body=body, headers=headers
        )

        if not webapps["webapps"]:
            raise Exception(
                UserExceptionMessages.DATASPACE_NOT_FOUND.format(
    ...
    async def get_webapp_content(self, webapp_id: str) -> WebappContent:
        ...
        webapp_content = await get_request(
            f"{self.__get_webapp_content_url.format(webapp_id=webapp_id)}", headers
        )
