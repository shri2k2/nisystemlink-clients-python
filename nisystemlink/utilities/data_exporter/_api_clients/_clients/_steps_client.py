...
class StepsClient:
    ...
    def __init__(self, api_key: str, systemlink_uri: str) -> None:
        ...
        self.__query_steps_url = f"{systemlink_uri}{BaseHttpRoutes.TEST_MONITOR_BASE_ROUTE}/query-steps"
    ...
    async def query_steps(self, steps_filter: str, results_filter: str) -> List[Dict]:
        ...
        """
        steps = []
        body = {"filter": steps_filter, "resultFilter": results_filter, "take": HttpConstants.TAKE, "orderBy": SystemLinkHttpKeys.UPDATED_AT}
        if self.__api_key:
            headers = {'x-ni-api-key': self.__api_key}
        response = await post_request(self.__query_steps_url, body=body, headers=headers)
